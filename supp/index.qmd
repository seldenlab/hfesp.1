...
```{r fig3, fig.cap="Color distance matrix (top) and NMDS (bottom) summarizing color variation in Perdiz arrow points, where the northern behavioral region is depicted in gray, and the southern behavioral region is depicted in orange with each point plotted in color space to visualize color variation."}
...
```{r fig4, fig.cap="Principal components analysis plot (PC1/PC2) for Hickory Fine Engraved and Smithport Plain bottles by firing category, where gray indicates bottles fired in a reduced environment, and orange from an oxidized environment, plotted with warp grids for the first two principal components."}
