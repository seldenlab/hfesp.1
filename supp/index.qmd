...
Color is afforded primacy in production-based inquiries, as it provides evidence associated with use, raw material variability, and firing method. This study is focused on the latter, and asks whether the intended firing method conditioned the morphology of Formative/Early Caddo bottles (CE 800 - 1200). Thirty three-dimensional (3-D) scans of Hickory Engraved and Smithport Plain bottles were collected from five museums and repositories, and provide 360-degree coverage of exterior colors associated with each vessel. Texture (color) files were used in a quantitative analysis of color where vessels were subsequently binned into two firing groups; oxidized and reduced. The 3-D meshes were then analyzed using the tools of geometric morphometrics to assess whether bottle shape and size differ by firing group. Results demonstrate that bottles from the two firing groups differ significantly in both color and shape, but not size. Thus, Caddo potters conditioned the shape of Hickory Engraved and Smithport Plain bottles based upon intended firing method. This finding provides evidence for two discrete, contemporary, and sympatric Caddo potter communities delimited by macroscopic attributes related to production.
...
<<<<<<< HEAD
Pairwise distances between histograms were computed using earth mover’s distance [@RN11201]. The distance matrix was exported for a permutational multivariate analysis of variance (MANOVA) using the `vegan` package to evaluate whether vessel color between firing categories differs (_permutations: 10,000; Rsq: 0.31881; **Pr(>F): 9.999e-05**_). A neighbor-joining tree was generated using the color distance matrix to illustrate color differences between oxidized and reduced vessels.
=======
Pairwise distances between histograms were computed using earth mover’s distance [@RN11201]. The distance matrix was exported for use in a permutational multivariate analysis of variance (MANOVA) using the `vegan` package to evaluate whether vessel color from each firing category differs (_permutations: 10,000; Rsq: 0.4324; **Pr(>F): 9.999e-05**_). A neighbor-joining tree was generated using the color distance matrix to illustrate color differences between the oxidized and reduced vessels.
>>>>>>> f4d7786d796ca6a463b040c30238f75619ea1842
